...
{
    std::string file_text = "This is just a test String!\nPlease disregard it!";

    // TODO: implement loading the file into a string
    //Opens the input file stream to receive data from the file
    std::ifstream aFile(filename);
    //Reads data from the file line by line and stores it all in file_text
...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
    ...
    //Attempts to open a new filestream with the passed in file name
    usedFile.open(filename, ios::out);
    ...
    //Assigns the year value, month value, and day value based on the current local time
    int year = 1900 + ltm.tm_year;
    int month = 1 + ltm.tm_mon;
    int day = ltm.tm_mday;

    //Checks to see if the file is open. If it is, the following text strings will be displayed as output
    if (usedFile.is_open()) {

        //Prints out the Student's name
        usedFile << "Student Name: " << student_name << std::endl;

        //Prints out the Current Time
        usedFile << "Time Stamp: (" << year << "-" << month << "-" << day << ")" << std::endl;

        //Prints out the key used for the encryption/decryption
        usedFile << "Key Used: " << key << endl;

        //Prints out the data from the file
        usedFile << "Data From File: " << data << endl;

        //Closes the file stream
        usedFile.close();
    }
    //If the file is not open, the user will be notified.
    else {
